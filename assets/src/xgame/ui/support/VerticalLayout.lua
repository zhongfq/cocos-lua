...
function VerticalLayout:do_layout()
    ...
    local pos = 0
    if self.content_v_align == UIAlign.CENTER then
        pos = (self.target.height - height) / 2
    elseif self.content_v_align == UIAlign.TOP then
    ...
    for i = #children, 1, -1 do
        ...
        if self.content_h_align == UIAlign.RIGHT then
            x = x + (rr - cr)
        elseif self.content_h_align == UIAlign.CENTER then
            x = x + (rl + rr) / 2  - (cl + cr) / 2 
        else
            x = x + (rl - cl)
        end
