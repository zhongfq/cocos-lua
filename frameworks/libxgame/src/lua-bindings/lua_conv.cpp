...
int auto_olua_push_cocos2d_PipelineDescriptor(lua_State *L, const cocos2d::PipelineDescriptor *value)
{
    if (value) {
        ...
        olua_push_cppobj(L, value->programState, "ccb.ProgramState");
        olua_setfield(L, -2, "programState");

        auto_olua_push_cocos2d_backend_BlendDescriptor(L, &value->blendDescriptor);
        olua_setfield(L, -2, "blendDescriptor");
    } else {
        lua_pushnil(L);
    }
...
void auto_olua_check_cocos2d_PipelineDescriptor(lua_State *L, int idx, cocos2d::PipelineDescriptor *value)
{
    ...
    cocos2d::backend::BlendDescriptor arg2;       /** blendDescriptor */

    olua_getfield(L, idx, "programState");
    if (!olua_isnoneornil(L, -1)) {
        olua_check_cppobj(L, -1, (void **)&arg1, "ccb.ProgramState");
        value->programState = (cocos2d::backend::ProgramState *)arg1;
    }
    lua_pop(L, 1);

    olua_getfield(L, idx, "blendDescriptor");
    auto_olua_check_cocos2d_backend_BlendDescriptor(L, -1, &arg2);
    value->blendDescriptor = (cocos2d::backend::BlendDescriptor)arg2;
    lua_pop(L, 1);
}

bool auto_olua_is_cocos2d_PipelineDescriptor(lua_State *L, int idx)
{
    return olua_istable(L, idx) && olua_hasfield(L, idx, "blendDescriptor");
}
...
void auto_olua_pack_cocos2d_PipelineDescriptor(lua_State *L, int idx, cocos2d::PipelineDescriptor *value)
{
    ...
    olua_check_cppobj(L, idx + 0, (void **)&arg1, "ccb.ProgramState");
    value->programState = (cocos2d::backend::ProgramState *)arg1;

    auto_olua_check_cocos2d_backend_BlendDescriptor(L, idx + 1, &arg2);
    value->blendDescriptor = (cocos2d::backend::BlendDescriptor)arg2;
}
...
{
    return olua_is_cppobj(L, idx + 0, "ccb.ProgramState") && auto_olua_is_cocos2d_backend_BlendDescriptor(L, idx + 1);
}
